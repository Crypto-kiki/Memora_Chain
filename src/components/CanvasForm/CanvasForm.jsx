...
function CanvasForm({
  ...
  useEffect(() => {
    //이미지 불러오기
    (async () => {
      const image2 = await loadImage(`${process.env.PUBLIC_URL}/image/logo2.png`);
      const image3 = await loadImage(`${process.env.PUBLIC_URL}/image/logo3.png`);
      ...
      image.onload = () => {
        ...
        if (iar < 0.9) {
          ...
          ctx.fillText(
            "MEMORIES",
            cw / 2.02,
            ch / 2 - ch / 18.18 - (rectheight2 * (2 - iar) - rectheight2) - 100
          );
          ctx.fillText(
            "IN CHAIN.",
            cw / 2.02,
