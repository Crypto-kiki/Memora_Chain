...
function CanvasForm6({
  ...
  useEffect(() => {
    // 이미지 그리
    (async () => {
      const image2 = await loadImage(`${process.env.PUBLIC_URL}/image/logo5.png`);
      const image3 = await loadImage(`${process.env.PUBLIC_URL}/image/logo4.png`);
    ...
    if (size[0] == 1) {
      image.onload = () => {
        ...
        ctx.drawImage(image3,cw / 2 - ((cw / 1.5) * 1.1) / 2 + 10 , ch- 135, 50,50 )
        ctx.fillStyle = "black";
        ctx.font = "37px SB";
        ctx.fillText(
          `MEMORIES`,
    ...
    if (size[0] == 2 || size[0] == 3) {
      image.onload = () => {
        ...
        //메인 사각형(그림) 그리기
        ctx.filter = "none";
        ctx.drawImage(image,cw / 2 - ( (cw / 1.39) * 0.8 * 1.4 / 2 ), ch / 2 -(ch / 1.1)/2, (cw / 1.39) * 0.8 * 1.4, 580);
        ctx.drawImage(image2,cw / 2 - ( (cw / 1.39) * 0.8 * 1.4 / 2 ) +(cw / 1.39) * 0.8 * 1.4-75, 55, 70,70)
